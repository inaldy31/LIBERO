...
- **Perbaikan update macet (stuck saat update)**: Saat worker unduhan update mati sebelum sempat menulis status akhir (crash, force-close OS, atau koneksi terputus), state pembaruan tertinggal di `downloading` tanpa pemulihan otomatis selama aplikasi masih berjalan, sehingga panel "Mengunduh pembaruan..." bisa diam di progress tertentu selamanya. Launcher kini mendeteksi state `downloading` basi (>120 detik tanpa pembaruan) pada dua titik — polling UI (5 detik) dan periodic checker (30 menit) — lalu memulihkannya ke `idle` agar update bisa dicoba lagi tanpa harus menutup aplikasi. Lihat `docs/release-notes-2.2.12.md` dan `_bmad-output/implementation-artifacts/investigations/stuck-saat-update-investigation.md` untuk diagnosis lengkap.
