// LIBERO: Alur ekstraksi PDF STOPPER Integrasi dan review hasil.
/* ── AI Auto-Isi PDF (AI) - Versi Menu Atas ──────────────────────────────── */
    ...
    `;
      ...
      function collectFields() {
        ...
        // tp-5 (Lingkungan), tp-6 (Riwayat Pidana), tp-7+ (naratif) TIDAK diisi AI.
        // tp-8 (Evaluasi Pembinaan) diisi karena ada tahapan pembinaan.
        var _allowedTabs = ['tp-0', 'tp-1', 'tp-2', 'tp-3', 'tp-4', 'tp-8'];
  ...
  `;
      ...
      function _stopperPdfFieldLabel(fid, fallback) {
        ...
        var labels = {
          ...
          'susunan-keluarga-penjamin': 'Susunan Keluarga Penjamin',
      ...
      function _stopperPdfFieldSection(fid, fallback) {
        ...
        if (key === 'riwayat-pernikahan-penjamin') return 'Kondisi Penjamin';
        ...
        try {
          ...
          if (res && res.ok && res.data) {
            ...
            for (var fid in res.data) {
              ...
              console.log('[Stopper] Processing fid:', fid, 'type:', typeof res.data[fid], 'isArray:', Array.isArray(res.data[fid]));
              ...
              if ((fid === 'susunan_keluarga' || fid === 'f-susunan_keluarga') && Array.isArray(val)) {
                ...
                try {
                  console.log('[Stopper] window.anggotaBersama exists?', !!window.anggotaBersama, 'type:', typeof window.anggotaBersama);
              ...
              if (fid === 'riwayat_pernikahan' && Array.isArray(val) && val.length > 0) {
                try {
                  var jmlEl = document.getElementById('f-jumlah-pernikahan');
                  if (jmlEl) {
                    jmlEl.value = String(val.length);
                    jmlEl.dispatchEvent(new Event('change', { bubbles: true }));
                    if (typeof updateMarriageFrames === 'function') updateMarriageFrames();
                    setTimeout(function () {
                      val.forEach(function (item, idx) {
                        var n = idx + 1;
                        var tNikah = document.getElementById('mf-' + n + '-tanggal-nikah');
                        var tTempat = document.getElementById('mf-' + n + '-tempat-nikah');
                        var tAgama = document.getElementById('mf-' + n + '-agama');
                        if (tNikah && item.tanggal) { tNikah.value = item.tanggal; tNikah.dispatchEvent(new Event('input', { bubbles: true })); }
                        if (tTempat && item.tempat) { tTempat.value = item.tempat; tTempat.dispatchEvent(new Event('input', { bubbles: true })); }
                        if (tAgama && item.agama) { tAgama.value = item.agama; tAgama.dispatchEvent(new Event('input', { bubbles: true })); }
                        filled++;
                      });
                    }, 300);
                  }
            ...
            // Auto-trigger onPenjaminChange setelah f-penjamin diisi AI
            var penjaminEl = document.getElementById('f-penjamin');
            if (penjaminEl && penjaminEl.value) {
              if (typeof onPenjaminChange === 'function') onPenjaminChange();
              console.log('[Stopper] f-penjamin diisi:', penjaminEl.value);
            }
            ...
            // Post-processing: Tambahkan /Penjamin di keterangan anggota keluarga
            // yang namanya sama dengan nama penjamin
            try {
              var _penjaminType = (penjaminEl && penjaminEl.value) || '';
              var _namaPenjamin = '';
              if (_penjaminType === 'Ayah') _namaPenjamin = (document.getElementById('f-nama-ayah') || {}).value || '';
              else if (_penjaminType === 'Ibu') _namaPenjamin = (document.getElementById('f-nama-ibu') || {}).value || '';
              else if (_penjaminType === 'Suami') _namaPenjamin = (document.getElementById('f-nama-suami') || {}).value || '';
              else if (_penjaminType === 'Istri') _namaPenjamin = (document.getElementById('f-nama-istri') || {}).value || '';
              else if (_penjaminType === 'Lainnya') _namaPenjamin = (document.getElementById('f-nama-penjamin') || {}).value || '';

              if (_namaPenjamin) {
                var _npLower = _namaPenjamin.trim().toLowerCase();
                console.log('[Stopper] Nama penjamin teridentifikasi:', _namaPenjamin);
                // Scan semua tabel keluarga, tambahkan /Penjamin jika nama cocok
                var _famArrays = [
                  { arr: window.anggotaBersama, tbody: 'tbody-bersama' },
                  { arr: window.anggotaKlien, tbody: 'tbody-klien' },
                  { arr: window.anggotaPenjamin, tbody: 'tbody-penjamin' }
                ];
                var _needRerender = [];
                _famArrays.forEach(function (fa) {
                  if (!fa.arr || !fa.arr.length) return;
                  fa.arr.forEach(function (member) {
                    var mName = (member.nama || '').trim().toLowerCase();
                    if (mName && mName === _npLower) {
                      var ket = (member.keterangan || '').trim();
                      if (ket.toLowerCase().indexOf('/penjamin') < 0 && ket.toLowerCase().indexOf('penjamin') < 0) {
                        member.keterangan = ket ? (ket + '/Penjamin') : 'Penjamin';
                        console.log('[Stopper] Menambahkan /Penjamin pada:', member.nama, '->', member.keterangan);
                      }
                    }
                  });
                  _needRerender.push(fa);
                });
                // Re-render tabel yang berubah
                _needRerender.forEach(function (fa) {
                  if (typeof _renderFamTable === 'function') _renderFamTable(fa.tbody, fa.arr);
                });
              }
            } catch (e) { console.error('[Stopper] Error post-processing penjamin tag:', e); }
      ...
      function inject() {
        ...
        var stopperSubtitle = 'Pilih dokumen untuk dianalisa AI. Semua dokumen bersifat opsional.<br><br><strong style="color:rgba(var(--ac,93,224,133),0.9)">INFO PENTING:</strong><br><span style="opacity:0.85">&bull; Jika Klien & Penjamin beda rumah, pisahkan upload <b>KK Klien</b> & <b>KK Penjamin</b>.</span><br><span style="opacity:0.85">&bull; Upload <b>KK Orang Tua</b> hanya jika data Ayah/Ibu tidak ada di KK Klien/Penjamin.</span>';
        ...
        function _buildModal(slots) {
          ...
          sub.innerHTML = 'Pilih dokumen untuk dianalisa AI. Semua dokumen bersifat opsional.<br><br><strong style="color:rgba(var(--ac,93,224,133),0.9)">INFO PENTING:</strong><br><span style="opacity:0.85">&bull; Jika Klien & Penjamin beda rumah, pisahkan upload <b>KK Klien</b> & <b>KK Penjamin</b>.</span><br><span style="opacity:0.85">&bull; Upload <b>KK Orang Tua</b> hanya jika data Ayah/Ibu tidak ada di KK Klien/Penjamin.</span>';
